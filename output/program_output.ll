; ModuleID = 'maindream'
source_filename = "maindream"
target datalayout = "e-m:o-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-apple-darwin20.1.0"

%dreamObj = type { i8*, %dreamObj**, i8*, %dreamObj*, %dreamObj**, %dreamObj**, %dreamObj*, i32, [100 x %dreamObj**] }

@nullDream = external global %dreamObj*
@line = external global i32
@0 = private unnamed_addr constant [7 x i8] c"Hello,\00", align 1
@1 = private unnamed_addr constant [2 x i8] c"a\00", align 1
@2 = private unnamed_addr constant [4 x i8] c"Mr.\00", align 1
@3 = private unnamed_addr constant [2 x i8] c"b\00", align 1
@4 = private unnamed_addr constant [5 x i8] c"name\00", align 1
@5 = private unnamed_addr constant [2 x i8] c"b\00", align 1
@6 = private unnamed_addr constant [2 x i8] c" \00", align 1
@7 = private unnamed_addr constant [5 x i8] c"name\00", align 1
@8 = private unnamed_addr constant [14 x i8] c"lambda_line_4\00", align 1
@9 = private unnamed_addr constant [9 x i8] c"@context\00", align 1
@10 = private unnamed_addr constant [5 x i8] c"name\00", align 1
@11 = private unnamed_addr constant [5 x i8] c"args\00", align 1
@12 = private unnamed_addr constant [2 x i8] c"c\00", align 1
@13 = private unnamed_addr constant [2 x i8] c"f\00", align 1
@14 = private unnamed_addr constant [2 x i8] c"f\00", align 1
@15 = private unnamed_addr constant [2 x i8] c"a\00", align 1
@16 = private unnamed_addr constant [5 x i8] c"Marc\00", align 1
@17 = private unnamed_addr constant [2 x i8] c"f\00", align 1
@18 = private unnamed_addr constant [2 x i8] c"c\00", align 1
@19 = private unnamed_addr constant [9 x i8] c"@context\00", align 1

define i32 @main() {
EntryBlock:
  %null_dream = load %dreamObj*, %dreamObj** @nullDream, align 8
  %obj_stack = alloca %dreamObj*, align 8
  %0 = call %dreamObj* @dreamObject()
  store %dreamObj* %0, %dreamObj** %obj_stack, align 8
  %obj = load %dreamObj*, %dreamObj** %obj_stack, align 8
  %obj_stack1 = alloca %dreamObj*, align 8
  %1 = call %dreamObj* @dreamObject()
  store %dreamObj* %1, %dreamObj** %obj_stack1, align 8
  %obj2 = load %dreamObj*, %dreamObj** %obj_stack1, align 8
  store i32 2, i32* @line, align 4
  %2 = call %dreamObj* @dreamStr(i8* getelementptr inbounds ([7 x i8], [7 x i8]* @0, i32 0, i32 0))
  %3 = call %dreamObj* @set_var(%dreamObj* %obj2, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @1, i32 0, i32 0), %dreamObj* %2)
  store i32 3, i32* @line, align 4
  %4 = call %dreamObj* @dreamStr(i8* getelementptr inbounds ([4 x i8], [4 x i8]* @2, i32 0, i32 0))
  %5 = call %dreamObj* @set_var(%dreamObj* %obj2, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @3, i32 0, i32 0), %dreamObj* %4)
  store i32 4, i32* @line, align 4
  %func_stack = alloca %dreamObj*, align 8
  %6 = call %dreamObj* @dreamFunc(i8* bitcast (%dreamObj* (%dreamObj*, %dreamObj*, ...)* @lambda_line_4 to i8*))
  store %dreamObj* %6, %dreamObj** %func_stack, align 8
  %func = load %dreamObj*, %dreamObj** %func_stack, align 8
  %7 = call %dreamObj* @set_var(%dreamObj* %obj2, i8* getelementptr inbounds ([14 x i8], [14 x i8]* @8, i32 0, i32 0), %dreamObj* %func)
  %8 = call %dreamObj* @set_var(%dreamObj* %func, i8* getelementptr inbounds ([9 x i8], [9 x i8]* @9, i32 0, i32 0), %dreamObj* %obj2)
  %9 = call %dreamObj* @dreamStr(i8* getelementptr inbounds ([5 x i8], [5 x i8]* @10, i32 0, i32 0))
  %func_stack3 = alloca %dreamObj*, align 8
  %10 = call %dreamObj* @dreamFunc(i8* bitcast (%dreamObj* (i32, ...)* @dreamList to i8*))
  store %dreamObj* %10, %dreamObj** %func_stack3, align 8
  %func4 = load %dreamObj*, %dreamObj** %func_stack3, align 8, !var_args !0
  %memberptr = getelementptr %dreamObj, %dreamObj* %func4, i32 0, i32 2
  %value_temp = load i8*, i8** %memberptr, align 8
  %11 = bitcast i8* %value_temp to %dreamObj* (...)*
  %12 = call %dreamObj* (...) %11(i32 1, %dreamObj* %9)
  %13 = call %dreamObj* @set_var(%dreamObj* %func, i8* getelementptr inbounds ([5 x i8], [5 x i8]* @11, i32 0, i32 0), %dreamObj* %12)
  %14 = call %dreamObj* @set_var(%dreamObj* %obj2, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @12, i32 0, i32 0), %dreamObj* %func)
  %15 = call %dreamObj* @set_var(%dreamObj* %obj, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @13, i32 0, i32 0), %dreamObj* %obj2)
  store i32 8, i32* @line, align 4
  %16 = call %dreamObj* (%dreamObj*, i8*, i32, ...) @get_var(%dreamObj* %obj, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @14, i32 0, i32 0), i32 1)
  %17 = call %dreamObj* (%dreamObj*, i8*, i32, ...) @get_var(%dreamObj* %16, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @15, i32 0, i32 0), i32 0)
  %18 = call %dreamObj* @dreamStr(i8* getelementptr inbounds ([5 x i8], [5 x i8]* @16, i32 0, i32 0))
  %19 = call %dreamObj* (%dreamObj*, i8*, i32, ...) @get_var(%dreamObj* %obj, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @17, i32 0, i32 0), i32 1)
  %20 = call %dreamObj* (%dreamObj*, i8*, i32, ...) @get_var(%dreamObj* %19, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @18, i32 0, i32 0), i32 0)
  %21 = call %dreamObj* (%dreamObj*, i8*, i32, ...) @get_var(%dreamObj* %20, i8* getelementptr inbounds ([9 x i8], [9 x i8]* @19, i32 0, i32 0), i32 1)
  %22 = call %dreamObj* @new_scope(%dreamObj* %21, i32 1)
  %scope_stack = alloca %dreamObj*, align 8
  store %dreamObj* %22, %dreamObj** %scope_stack, align 8
  %new_scope = load %dreamObj*, %dreamObj** %scope_stack, align 8
  %memberptr5 = getelementptr %dreamObj, %dreamObj* %20, i32 0, i32 2
  %value_temp6 = load i8*, i8** %memberptr5, align 8
  %23 = bitcast i8* %value_temp6 to %dreamObj* (...)*
  %24 = call %dreamObj* (...) %23(%dreamObj* %new_scope, %dreamObj* %18)
  %func_stack7 = alloca %dreamObj*, align 8
  %25 = call %dreamObj* @dreamFunc(i8* bitcast (void (i32, %dreamObj*, ...)* @print to i8*))
  store %dreamObj* %25, %dreamObj** %func_stack7, align 8
  %func8 = load %dreamObj*, %dreamObj** %func_stack7, align 8, !var_args !0
  %memberptr9 = getelementptr %dreamObj, %dreamObj* %func8, i32 0, i32 2
  %value_temp10 = load i8*, i8** %memberptr9, align 8
  %26 = bitcast i8* %value_temp10 to %dreamObj* (...)*
  %27 = call %dreamObj* (...) %26(i32 2, %dreamObj* %17, %dreamObj* %24)
  ret i32 0
}

declare void @print(i32, %dreamObj*, ...)

declare void @gc(%dreamObj*)

declare void @free(i8*)

declare void @dream_log(%dreamObj*)

declare %dreamObj* @dreamList(i32, ...)

declare %dreamObj* @count(%dreamObj*)

declare %dreamObj* @pointer(%dreamObj*)

declare %dreamObj* @ptr(%dreamObj*)

declare i32 @printjf(i8*, ...)

declare %dreamObj* @dreamObject()

declare %dreamObj* @set_var(%dreamObj*, i8*, %dreamObj*)

declare %dreamObj* @set_var_c(%dreamObj*, i8*)

declare %dreamObj* @get_var(%dreamObj*, i8*, i32, ...)

declare %dreamObj* @equals_c(%dreamObj*, %dreamObj*)

declare %dreamObj* @contains_c(%dreamObj*, %dreamObj*)

declare %dreamObj* @dreamStr(i8*)

declare %dreamObj* @dreamInt(i32)

declare %dreamObj* @dreamBool(i32)

declare %dreamObj* @dreamFunc(i8*)

declare %dreamObj* @make_dream(i8*, ...)

declare %dreamObj* @new_scope(%dreamObj*, i32)

declare %dreamObj* @copy(%dreamObj*)

declare %dreamObj* @deep_copy(%dreamObj*)

declare %dreamObj* @shallow_copy(%dreamObj*)

declare void @dict(%dreamObj*)

declare void @inherit(%dreamObj*)

declare void @makeText(%dreamObj*)

declare %dreamObj* @add_c(%dreamObj*, %dreamObj*)

declare %dreamObj* @set_parent(%dreamObj*, %dreamObj*)

declare void @merge(%dreamObj*, %dreamObj*)

declare i8* @ctype(%dreamObj*)

declare void @display()

declare void @native_int(i32)

declare void @apply_vargs(%dreamObj*, %dreamObj*, %dreamObj*)

declare void @native_test(i32)

declare void @check(%dreamObj*, %dreamObj*)

declare void @input()

declare void @clear()

define %dreamObj* @lambda_line_4(%dreamObj* %scope, %dreamObj* %name, ...) !arg_names !1 {
EntryBlock:
  %null_dream = load %dreamObj*, %dreamObj** @nullDream, align 8
  %alloctmp = alloca %dreamObj*, align 8
  store %dreamObj* %name, %dreamObj** %alloctmp, align 8
  %varName = load %dreamObj*, %dreamObj** %alloctmp, align 8
  %0 = call %dreamObj* @set_var(%dreamObj* %scope, i8* getelementptr inbounds ([5 x i8], [5 x i8]* @4, i32 0, i32 0), %dreamObj* %varName)
  %func_stack = alloca %dreamObj*, align 8
  store %dreamObj* %scope, %dreamObj** %func_stack, align 8
  %func = load %dreamObj*, %dreamObj** %func_stack, align 8
  store i32 5, i32* @line, align 4
  %1 = call %dreamObj* (%dreamObj*, i8*, i32, ...) @get_var(%dreamObj* %func, i8* getelementptr inbounds ([2 x i8], [2 x i8]* @5, i32 0, i32 0), i32 1)
  %2 = call %dreamObj* @dreamStr(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @6, i32 0, i32 0))
  %3 = call %dreamObj* @add_c(%dreamObj* %1, %dreamObj* %2)
  %4 = call %dreamObj* (%dreamObj*, i8*, i32, ...) @get_var(%dreamObj* %func, i8* getelementptr inbounds ([5 x i8], [5 x i8]* @7, i32 0, i32 0), i32 1)
  %5 = call %dreamObj* @add_c(%dreamObj* %3, %dreamObj* %4)
  ret %dreamObj* %5
}

; Function Attrs: nounwind
declare void @llvm.stackprotector(i8*, i8**) #0

attributes #0 = { nounwind }

!0 = !{!"1"}
!1 = !{!"name"}
